...
### Model GLM1: Basic GLM without interactions
    c.glm1 <- glm(ClaimNb ~ VehPowerGLM + VehAgeGLM + BonusMalusGLM
              + VehBrand + VehGas + DensityGLM + Region + AreaGLM
              + DrivAge,
              data=learn, offset=log(Exposure), family=poisson())
summary(c.glm1)

learn$fitGLM1 <- fitted(c.glm1)
test$fitGLM1 <- predict(c.glm1, newdata=test, type="response")
dat$fitGLM1 <- predict(c.glm1, newdata=dat2, type="response")
...
### Model GLM2: Basic GLM without interactions
    c.glm2 <- glm(ClaimNb ~ VehPowerGLM + VehAgeGLM + BonusMalusGLM
              + VehGas + DensityGLM + AreaGLM
              + DrivAge,
              data=learn, offset=log(Exposure), family=poisson())
summary(c.glm2)

learn$fitGLM2 <- fitted(c.glm2)
test$fitGLM2 <- predict(c.glm2, newdata=test, type="response")
dat$fitGLM2 <- predict(c.glm2, newdata=dat2, type="response")
...
### Model GLM3: Basic GLM without interactions
    c.glm3 <- glm(ClaimNb ~ VehPowerGLM + VehAgeGLM + BonusMalusGLM
              + VehGas + DensityGLM
              + DrivAge,
              data=learn, offset=log(Exposure), family=poisson())
summary(c.glm3)

learn$fitGLM3 <- fitted(c.glm3)
test$fitGLM3 <- predict(c.glm3, newdata=test, type="response")
dat$fitGLM3 <- predict(c.glm3, newdata=dat2, type="response")
...
tree <- rpart(ClaimNb, data = dat, cp = .02)
